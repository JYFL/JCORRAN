...
TClonesArray& AliJJetFinder::RecoJets( TObjArray * inputlist, vector<int>*sublist=NULL){
    ...
    for( int i=0;i<nTracks;i++ ){
        int itrack = sublist?(*sublist)[i]:i;
        // skip if not charged
        AliJBaseTrack &l = *((AliJBaseTrack*)fInputList->At(itrack));
        if( fUseCharged && (!l.GetCharge()) ) continue; //TODO
    ...
    for( unsigned int i=0;i< jets.size();i++ ){
        ...
        for( unsigned int ic=0;ic< constituents.size();ic++ ){
            int index = constituents[ic].user_index();
...
AliJJetFinder::JETS AliJJetFinder::find () {
  ...
  fastjet::AreaDefinition fArea_def;
    ...
    if(fJetalg==kantikt) {
        jet_def = fastjet::JetDefinition (fastjet::antikt_algorithm, fR, pt_scheme, Best);
