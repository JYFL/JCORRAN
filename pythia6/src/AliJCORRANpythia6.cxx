...
ClassImp(AliJCORRANpythia6)

AliJCORRANpythia6::AliJCORRANpythia6() :
    TObject(),
    fExecLocal(kTRUE),
    fFirstEvent(kTRUE),
    fcard(0),
...
AliJCORRANpythia6::AliJCORRANpythia6(Bool_t execLocal) :
	TObject(),
...
AliJCORRANpythia6::AliJCORRANpythia6(const AliJCORRANpythia6& obj){
	// copy constructor
}

AliJCORRANpythia6& AliJCORRANpythia6::operator=(const AliJCORRANpythia6& obj){
	// copy constructor
	return *this;
}


void AliJCORRANpythia6::Initialize() const{

}

void AliJCORRANpythia6::UserCreateOutputObjects(){
	// local init
    //---------------------
    //Pythia initialization
    //---------------------
    fPythia = new TPythia6;   // Generator.
   /***********************************
     * PYTUNE : PERUGIA0(320)
	 * Atlas-CSC (306)
	 ***********************************/
	Int_t pytune   = 320;
	if( pytune > -1 ) { // PERUGIA0 -> 320
		//fPythia->Pytune( pytune );
		/***********************************
		 * MANUAL Setting
		 ***********************************/
	} else {
		...
		fPythia->SetMSTP(51,7);//PDF lib
		fPythia->SetMSTP(52,1);//PDF Type
		fPythia->SetMSEL(1);
		fPythia->SetMSTP(82,4);//D=4, 0 -> soft only; 3 -> var imp. par p.313 (def=1)

		fPythia->SetMSTP(91, 1);
		fPythia->SetPARP(91, 2.0); // mean kt
		fPythia->SetPARP(93, 15.0);//card->KTupperCut);
		fPythia->SetPARJ(21, 0.36);//card->meanJTY);

		fPythia->SetPARP(67, 1);//card->virtuality );
		fPythia->SetMSTJ(1,  2);//D=1, 0=no fragmentation,1= LUND 2=independent (akiba 2)fragmentation scheme, see also MSTP(111) ?
		fPythia->SetMSTJ(2,  1);//int(card->doFragGlue));
		fPythia->SetMSTJ(3,  1);//int(card->fragMomConv));   // mom conserv in fragmentation scheme, Jia has 4
		fPythia->SetMSTP(61, 1);
		fPythia->SetMSTJ(42, 1); // angular ordering (default=2, 1 for no ordering)
		fPythia->SetMSTP(71, 1);       // 0 -> no final   state showers;  1 -> default
		fPythia->SetMSTP(71, 1);
		fPythia->SetMSTP(125, 1); // p.199 pylist setup
		fPythia->SetCKIN(3, 0.);
		fPythia->SetCKIN(4, 10000);
	}
	/***********************************
	 * Additional Setting
	 ***********************************/
    if((frandom_seed>=0)&&(frandom_seed<=900000000)){
        fPythia->SetMRPY(1,  frandom_seed);      // set seed
        fPythia->SetMRPY(2,  0);     // use new seed
        cout<<"Random Seed : "<<frandom_seed<<endl;
    }

	// Extract settings to be used in the main program.
   // TODO : card or argument
    char  targ[10] = "p";
    char  proj[10] = "p";        // particle species
    fPythia->Initialize("CMS", targ, proj, fSQRTS);
	...
	fdmg = new JPythia6DataManager(fcard, fhistos);
...
void AliJCORRANpythia6::UserExec(){
	// event loop
...
void AliJCORRANpythia6::Terminate() {
	// termination
...
particleType  AliJCORRANpythia6::GetParticleType(char *inchar){
	// part type
...
double AliJCORRANpythia6::DeltaPhi(double phi1, double phi2) {
	// dphi
...
void AliJCORRANpythia6::ScaleNotEquidistantHisto(TH1D *hid, const double sc=1){
	// histo scaler
...
void AliJCORRANpythia6::SavePythiaSetting(){
	// read card
/*
	ifstream infile;
	TString ms;
...
void AliJCORRANpythia6::FillXtHistos(TClonesArray *finputList, AliJTrackCounter *lPtrackCounter){
